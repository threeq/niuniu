#!/usr/bin/env bash
# Package the desktop-v2 (Tauri) Mach-O binary into a directly-runnable macOS .app bundle
...
usage() {
    cat <<EOF
Usage: $0 \\
    --binary <path>            \\  # the Mach-O produced by cargo build
    --icon <path>              \\  # icon.icns
...
# (`make package-personal-v2-darwin` on a dev Mac with no certs) behaves exactly as
...
# the Hardened Runtime enabled, which notarization requires. The server/mcp
# sidecars are shipped *inside* the main binary via include_bytes! (extracted to
# ~/.niuniu at runtime), so the bundle has exactly one executable to sign here.
