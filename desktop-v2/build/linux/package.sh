#!/usr/bin/env bash
# Package the desktop-v2 (Tauri) Linux binary into a portable AppImage — a
# single self-mounting executable users can `chmod +x` and double-click to run,
# with launcher integration baked in. This is the Linux equivalent of the macOS
# .dmg.
#
# Output:
...
#   AppDir/
#   ├── niuniu-desktop-v2        (the ELF binary produced by cargo build)
#   ├── niuniu-desktop-v2.desktop (desktop entry — drives launcher integration)
#   ├── niuniu-desktop-v2.png    (app icon, named to match the desktop Icon=)
...
# system (Tauri/wry runtime deps), same contract as a .deb. The build host
...
usage() {
    cat <<EOF
Usage: $0 \\
    --binary <path>              \\  # the ELF binary produced by cargo build
    --icon <path>                \\  # PNG icon (256px recommended)
    --display-name <"Niuniu Desktop">  \\  # used in .desktop Name=
    --version <vX.Y.Z>           \\
    --arch <amd64|arm64>         \\
    --artifact-base <niuniu-desktop-v2-vX.Y.Z>  \\  # filename stem
    --output-dir <bin/>
EOF
    exit 1
}
...
# Binary
cp "$BINARY" "$APPDIR/niuniu-desktop-v2"
chmod +x "$APPDIR/niuniu-desktop-v2"
...
cat > "$APPDIR/niuniu-desktop-v2.desktop" <<DESKTOP
[Desktop Entry]
Version=1.0
Type=Application
Name=${DISPLAY_NAME}
Comment=AI Workstation — run parallel AI agent sessions
Exec=niuniu-desktop-v2
Icon=${ICON_NAME}
Categories=Development;Utility;
Terminal=false
StartupWMClass=niuniu-desktop-v2
DESKTOP
