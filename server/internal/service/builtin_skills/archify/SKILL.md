...
This copy is **vendored into niuniu** and is updated only by a repo-side vendor
refresh (see `VENDOR.md`). The upstream update checker has been removed to keep
the skill fully offline — never look for it, never mention update checks, and
never fetch a release manifest. Just do the user's task.
