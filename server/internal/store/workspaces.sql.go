...
const setWorkspaceEnvProviderGroup = `-- name: SetWorkspaceEnvProviderGroup :exec
UPDATE workspaces SET env_provider_group = ?, env_provider_id = NULL WHERE id = ?
`

type SetWorkspaceEnvProviderGroupParams struct {
	EnvProviderGroup string `json:"env_provider_group"`
	ID               int64  `json:"id"`
}
...
func (q *Queries) SetWorkspaceEnvProviderGroup(ctx context.Context, arg SetWorkspaceEnvProviderGroupParams) error {
	_, err := q.db.ExecContext(ctx, setWorkspaceEnvProviderGroup, arg.EnvProviderGroup, arg.ID)
	return err
}
