// Package docextract is pure-Go text extraction for binary document formats
// (PDF / docx / xlsx / pptx).
//
// Originally written for the MCP read-accel feature (issue #281) and living in
// that binary's package main; promoted to a shared internal package so the
// knowledge-base ingest path can reuse it. Two consumers today:
//   - cmd/niuniu-mcp (read_document / ingest_directory), via thin aliases
//   - internal/service KB ingest + document browse, via IsSupported/PlainText
//
...
// loop-prevention). Nothing here ever panics out to the caller: the PDF path in
// particular is wrapped in recover() because ledongthuc/pdf can panic on
// malformed font tables.
package docextract
