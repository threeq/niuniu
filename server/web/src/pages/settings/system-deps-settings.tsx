...
const toolLabels: Record<string, { label: string; downloadUrl: string }> = {
  ...
  'cursor-agent': { label: 'Cursor Agent CLI', downloadUrl: 'https://cursor.com/docs/cli/overview' },
...
function commandFor(tool: string, info: SystemDepsInfo): string {
  ...
  // cursor-agent ships as a versioned tarball behind Cursor's own install
  // script, so there is no package-manager form. Mirrors commandFor() in
  // internal/service/system_deps.go, which returns no command and lets Install
  // fall back to the docs URL rather than piping a remote script into a shell.
  if (tool === 'cursor-agent') return ''
